...
- **Optional but recommended for full stack:** Install from OperatorHub before or in parallel if not already present:
  ...
  - **cert-manager Operator for Red Hat OpenShift** (required by Connectivity Link and Leader Worker Set)
...
│   │   ├── leaderworkerset/     # Leader Worker Set (multi-node AI/ML inference)
...
├── overlays/
│   ├── 01-operators/             # NFD, NVIDIA, Connectivity Link, Leader Worker Set, RHOAI
...
# Wait for NFD, NVIDIA, Connectivity Link, Leader Worker Set, and RHOAI operators to be installed and ready (oc get csv -A).
...
| **Leader Worker Set** | Manages multi-node AI/ML inference (LeaderWorkerSet API); one leader + workers per group. Requires cert-manager. |
